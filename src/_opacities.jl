...
function _inner_compute_chunk_extended!(range, chi, B::Nothing, dBdT::Nothing, kappa_data, src_data, weights, coefs_T, coefs_Rho, ρ, T, grid_T)
    n_shells = length(coefs_T)
    nT = length(grid_T)
...
function _inner_compute_chunk!(range, chi, B::Nothing, dBdT::Nothing, kappa_data, λ, weights, coefs_T, coefs_Rho, ρ, T)
    n_shells = length(coefs_T)
...
function generate_eos_identifier(feh::Float64, comp_dict::Dict{Symbol, Float64})
    id_parts = ["z_$(feh)"]
...
function get_or_compute_eos(
    ...
    composition::String;
    out_dir::String,
    modelatmosfolder::String = "input_multi3d/test_opac_table/",
    alpha::Float64 = 0.0,
    abund::String = "./input_multi3d/abund/abund_magg",
    ...
    eos_id = generate_eos_identifier(feh, comp_dict)
    ...
    λ_file_opt = use_lambda_file && isfile(lambda_file) ? lambda_file : nothing
